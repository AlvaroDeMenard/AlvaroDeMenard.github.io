...
Author: Said Achmiz
2019-03-11
license: MIT (derivative of footnotesjs, which is PD)
*/
...
function ridiculousWorkaroundsForBrowsersFromBizarroWorld() {
    ...
    if (!GW.isFirefox) {
        GW.sidenotes.viewportWidthBreakpointMediaQueryString = `(max-width: 155ch)`;
        ...
        /*  This should match the "max-width" property of the "body" element.
            */
        GW.maxBodyWidthInCharacterUnits = 85;
        ...
        let viewportWidthBreakpointInPixels = 100 * widthOfCharacterUnit;
...
function updateSidenotePositions() {
    ...
    /*  Position left sidenote column so top is flush with top of first
        ...
        */
    let markdownBody = document.querySelector(".content_container");
    var firstFullWidthBlock = markdownBody.children[0];
...
function constructSidenotes() {
    ...
    for (var i = 0; i < GW.sidenotes.footnoteRefs.length; i++) {
        ...
        sidenote.innerHTML = "<div class='sidenote-outer-wrapper'><div class='sidenote-inner-wrapper'>" +
                             (referencedFootnote ? referencedFootnote.querySelector(".footnote-contents").innerHTML : "Loading sidenote contents, please wait…")
                             + "</div></div>";
    ...
    for (var i = 0; i < GW.sidenotes.footnoteRefs.length; i++) {
        ...
        sidenoteSelfLink.textContent = GW.sidenotes.footnoteRefs[i].innerText;
